...
const WarehousePanel: React.FC<Props> = ({ showToast, products, setProducts, costItems, setCostItems, siteConfig, isMediaUrl, mode = 'all' }) => {
  ...
  const loadProcessingTypes = useCallback(async () => {
    setPtLoading(true);
    const makeHeaders = (op: 'read' | 'create') => {
    ...
    try {
      ...
      const missingStandard = STANDARD_PROCESSING_CODES.some(code => !rows.some((r: any) => r.code === code));
      if (missingStandard) {
        const seedRes = await fetch('/api/processing-types', {
          method: 'POST',
          headers: makeHeaders('create'),
          body: JSON.stringify({ action: 'seed_standard' }),
        });
        const seedJson = await seedRes.json().catch(() => ({}));
        if (!seedRes.ok || !seedJson.ok) throw new Error(seedJson.error || `HTTP ${seedRes.status}`);
        rows = await readRows();
      }
      setProcessingTypes(rows.map((r: any) => mapProcessingTypeRow(r)));
    } catch (err: any) {
      showToast(`加工方式載入失敗：${err.message}`, 'error');
    }
    setPtLoading(false);
  }, [showToast]);
  ...
  useEffect(() => {
    ...
    if (subTab === 'product_tree') loadProcessingTypes();
        ...
        {([
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-sm font-black transition-all flex items-center gap-2 ${
              subTab === tab.id
                ? tab.id === 'product_tree' ? 'bg-teal-600 text-white shadow-lg'
      ...
      {subTab === 'product_tree' && (() => {
        ...
        return (
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(['all', 'retail', 'wholesale'] as const).map(ch => (
                <button key={ch} type="button" onClick={() => setCostsChannelFilter(ch)}
                  ...
                  {ch === 'all' ? '子項通路·全部' : ch === 'retail' ? '零售' : '批發'}
                </button>
              ))}
            ...
            {orphans.length > 0 && (
              <div className="bg-amber-50/60 border border-amber-100 rounded-[2rem] p-5 space-y-3">
                <div className="flex items-center gap-2">
                  <AlertTriangle size={18} className="text-amber-600 shrink-0" />
                  <div>
                    <h4 className="font-black text-sm text-amber-900">尚未掛母料的產品（{orphans.length}）</h4>
                    <p className="text-[10px] text-amber-800/90 font-bold">選母料後按「掛上」即可出現在樹狀子表（仍受上方通路篩選）。</p>
                  </div>
                </div>
                <div className="space-y-2 max-h-72 overflow-y-auto">
                  {orphans.map(p => (
                    <div key={p.id} className="flex flex-wrap items-center gap-2 bg-white rounded-xl border border-amber-100/80 px-3 py-2">
                      <span className="font-bold text-slate-800 text-xs flex-1 min-w-[120px]">{p.name}</span>
                      <select defaultValue="" className="px-2 py-1.5 rounded-lg border border-slate-200 text-xs font-bold min-w-[160px] flex-1" id={`tree-link-${p.id}`}>
                        <option value="">— 選母料 —</option>
                        {[...ingredients].sort((a, b) => a.name.localeCompare(b.name)).map(ing => (
                          <option key={ing.id} value={ing.id}>{ing.name}</option>
                        ))}
                      </select>
                      <button type="button" onClick={() => {
                        const el = document.getElementById(`tree-link-${p.id}`) as HTMLSelectElement;
                        const v = el?.value || '';
                        void assignProductToIngredient(p.id, v);
                      }} className="px-3 py-1.5 bg-teal-600 text-white rounded-lg text-[10px] font-black shrink-0">掛上</button>
                    </div>
                  ))}
                </div>
              </div>
