...
export const mapIngredientRowToIngredient = (row: SupabaseIngredientRow): Ingredient => ({
  ...
  netContentVolume: row.net_content_volume ?? undefined,
  netContentUnit: (['g', 'kg', 'lb', 'catty'].includes(row.net_content_unit ?? '') ? row.net_content_unit as 'g' | 'kg' | 'lb' | 'catty' : undefined),
...
export const mapIngredientToRow = (ing: Ingredient): SupabaseIngredientRow => ({
  ...
  net_content_volume: ing.netContentVolume ?? null,
  net_content_unit: ing.netContentUnit ?? null,
